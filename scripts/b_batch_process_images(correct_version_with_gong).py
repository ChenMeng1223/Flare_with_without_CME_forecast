...
# ===== 批量绘图模式配置（直接改这里即可，无需命令行传入） =====
# 可选: 'training' = 使用 configs/data_config.yaml 中的 data.events_file
#      'new'      = 只绘制下面 NEW_EVENTS_FILE 指定的新增事件表
EVENTS_MODE = 'new'  # 'training' or 'new'
NEW_EVENTS_FILE = r'data/raw/events_plot.xlsx'
# ======================================================
...
class FITSImageProcessor:
    ...
    BAND_CONFIG = {
        94: {'dmax': 120.0, 'dmin': 1.0, 'cmap': 'sdoaia94', 'label': '94Å'},
        171: {'dmax': 2500.0, 'dmin': 5.0, 'cmap': 'sdoaia171', 'label': '171Å'},
        193: {'dmax': 3200.0, 'dmin': 10.0, 'cmap': 'sdoaia193', 'label': '193Å'},
        304: {'dmax': 160.0, 'dmin': 1.0, 'cmap': 'sdoaia304', 'label': '304Å'},
    }
    ...
    def __init__(self, config: Dict):
        ...
        # 计算目标太阳半径像素值（取输出尺寸一半的90%，留少量边距）
        self.target_solar_radius_pix = min(self.target_size[0], self.target_size[1]) / 2.0 * 0.9
        # 初始化sunpy可用性标记
    ...
    def find_fits_files(self, download_dir: Path) -> Dict[str, List[Path]]:
        ...
        for modality_dir in download_dir.iterdir():
            ...
            if modality_dir.is_dir() and modality_dir.name in self.modalities:
                ...
                # 查找该目录下所有 FITS 文件（含 .fits 与 .fits.fz）
                fits_list = list(modality_dir.glob('*.fits'))
                fits_list.extend(list(modality_dir.glob('*.fits.fz')))
                fits_files[modality] = fits_list
                # 按文件名排序（保证按时间顺序处理）
    ...
    def plot_halpha_aligned(self, fits_path: Path, output_path: Path, slice_index: Optional[int] = 69, use_color: bool = True) -> bool:
        ...
        try:
            ...
            with self.fits.open(str(fits_path), mode='readonly') as hdu_list:
                if is_gong:
                    # GONG: 优先使用 HDU[1]（data/plot_Halpha_GONG.py 的处理方式）
                    if len(hdu_list) > 1 and hdu_list[1].data is not None:
                        gong_data = hdu_list[1].data
                    elif hdu_list[0].data is not None:
                        gong_data = hdu_list[0].data
                    else:
                        return False
                    if gong_data.ndim != 2:
                        return False
                    image_data = gong_data.astype(np.float32)
                    original_exposure = self._get_exposure_time(fits_path)###################################################尝试处理GONG数据的曝光时间###################################################
                    print(f"original_exposure: {original_exposure}")
                    image_data = image_data / original_exposure
                else:
                    # CHASE: 保持原先流程（RSM...HA.fits）
            ...
            image_data = np.flipud(image_data)   # Y轴 上下翻转（垂直）
            # 图像对齐并缩放到目标尺寸
            ...
            # 彩色绘制
            if use_color:
                # CHASE 与 GONG 使用不同拉伸（GONG参考 data/plot_Halpha_GONG.py）
                if is_gong:
                    vmin = float(np.percentile(image_data, 0))
                    vmax = float(np.percentile(image_data, 100))
                    if vmax <= vmin:
                        vmax = vmin + 1.0
                else:
                    vmin = 0.0
...
def _is_event_row_empty(row: pd.Series) -> bool:
    """检查事件表中的一行是否完全为空"""
...
def _parse_event_time_column(time_value, base_date) -> datetime:
    """解析事件时间列，兼容 HH:MM、HH:MM:SS 和 (+Nday) 标记"""
    ...
    if isinstance(time_value, str):
        ...
        for offset in (1, 2):
            ...
            if marker in time_text:
                day_offset = offset
                time_text = time_text.replace(marker, "").strip()
                break
        ...
        parsed_dt = pd.to_datetime(time_text)
        if day_offset:
            parsed_dt = parsed_dt + pd.Timedelta(days=day_offset)
        return parsed_dt.to_pydatetime()

    return datetime.combine(base_date, time_value)


def _combine_event_date_time(df: pd.DataFrame) -> pd.DataFrame:
    """将 DATE + start_time/peak_time/end_time 组合成 ISO 时间字符串"""
    if "DATE" not in df.columns:
        return df

    df = df.copy()
    for idx, row in df.iterrows():
        if _is_event_row_empty(row):
            continue

        date_text = str(row["DATE"])
        if date_text.lower() == "nan" or not date_text or date_text == "None":
            continue
        ...
        for col in ("start_time", "peak_time", "end_time"):
            if col not in df.columns:
                continue
            try:
                df.at[idx, col] = _parse_event_time_column(row[col], base_date).isoformat()
            except Exception:
...
def _generate_event_ids_like_downloader(df: pd.DataFrame) -> pd.Series:
    """按下载脚本的规则生成 event_id: EVT_YYYYMMDD_HHMMSS_计数器"""
    event_ids = []
    time_count = {}
    ...
    return pd.Series(event_ids, index=df.index, dtype='string')
...
def load_events_metadata(events_file: str) -> pd.DataFrame:
    ...
    # 自动生成缺失或无效的 event_id，规则与下载脚本保持一致
    if 'event_id' not in df.columns:
        df.insert(0, 'event_id', pd.NA)
        logger.warning("事件文件缺少 event_id 列，已按下载脚本规则自动生成")

    df_for_event_id = _combine_event_date_time(df)
    generated_ids = _generate_event_ids_like_downloader(df_for_event_id)

    event_ids = df['event_id'].astype('string').str.strip()
    invalid_mask = event_ids.isna() | (event_ids == '') | (event_ids.str.lower() == 'nan')
    invalid_count = int(invalid_mask.sum())
    if invalid_count > 0:
        logger.warning(f"事件表中发现 {invalid_count} 条无效 event_id（空值/NaN），已按下载脚本规则自动生成")

    df['event_id'] = event_ids.mask(invalid_mask, generated_ids).astype(str)

    duplicated_mask = df['event_id'].duplicated(keep=False)
    if duplicated_mask.any():
        duplicate_count = int(duplicated_mask.sum())
        logger.warning(f"事件表中发现 {duplicate_count} 条重复 event_id，已自动追加行号保证唯一")
        df.loc[duplicated_mask, 'event_id'] = [
            f"{event_id}_{idx + 1:06d}"
            for idx, event_id in zip(df.index[duplicated_mask], df.loc[duplicated_mask, 'event_id'])
        ]

    if df.empty:
        raise ValueError(f"事件文件中没有事件记录: {events_file}")
...
def resolve_events_file(config: Dict) -> str:
    """解析本次批量绘图要使用的事件文件。"""
    training_events_file = config['data']['events_file']
    if EVENTS_MODE == 'training':
        return training_events_file

    if EVENTS_MODE == 'new':
        if not NEW_EVENTS_FILE:
            raise ValueError('当 EVENTS_MODE = "new" 时，必须在代码顶部填写 NEW_EVENTS_FILE')
        return str(NEW_EVENTS_FILE)

    raise ValueError(f'不支持的 EVENTS_MODE: {EVENTS_MODE}')
...
def main():
    ...
    # 从代码顶部配置中获取事件元数据文件路径
    events_file = resolve_events_file(config)
    logger.info(f"绘图模式: {EVENTS_MODE}")
